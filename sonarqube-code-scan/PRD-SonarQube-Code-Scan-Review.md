...
- 支持流水线已有 SonarQube 代码扫描任务选择代码扫描配置。
- 支持质量门禁失败时按配置决定是否阻断流水线。
- 支持扫描完成后发送扫描摘要、质量门禁结果和 SonarQube 详情链接。
...
5. 系统初始化扫描配置和权限。
6. 用户进入配置页补充扫描路径、触发规则、质量门禁和通知模板。
...
- 扫描配置可有多个拥有人，任一拥有人均可修改配置。
- 关联应用 Owner 可修改该应用关联的扫描配置。
- 关联应用普通成员仅可查看。
- 创建人作为审计信息保留，不作为当前权限字段。
- 扫描配置发生人工交接或应用 Owner 转移后，新拥有人 / 新 Owner 接管配置修改权限。
