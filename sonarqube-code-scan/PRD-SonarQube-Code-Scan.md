...
- 任务名称下方选择代码扫描配置，配置范围为当前流水线应用和流水线代码源仓库下当前用户可用的扫描配置。
- 没有可用配置时，在字段下方展示快速创建入口；点击后打开新建扫描配置弹窗，并默认带入当前流水线代码源仓库和当前流水线应用。
- 任务参数用于覆盖本条流水线的扫描行为。
...
| 代码扫描配置 | 必填；从当前流水线应用和代码源仓库下可用配置中选择；无配置时支持快速创建，并复用新建扫描配置弹窗 |
| 语言类型 | Java、Scala 等 |
| 模块名称 | 代码模块名称 |
| 指定子模块 | 是否指定子模块 |
| 源代码路径 | 如 src |
...
- 创建后不支持直接修改绑定仓库。
- 如需更换仓库，需新建扫描配置。
- 配置流水线 SonarQube 代码扫描任务时，需要选择代码扫描配置；配置范围为当前流水线应用和流水线代码源仓库下当前用户可用的扫描配置。
- 若没有可用配置，任务抽屉提供快速创建；快速创建复用新建扫描配置弹窗，默认带入当前流水线代码源仓库和当前流水线应用，并复用同仓库 SonarQube 项目。
- 原因：修改仓库会影响 SonarQube 项目、扫描历史、权限同步、流水线引用关系，属于高风险迁移场景，不纳入本期范围。

### 6.3 存量代码扫描配置

- 存量配置自动兼容，无需业务方重建；上线后按仓库创建或绑定 SonarQube 项目。
- 存量配置的扫描路径、触发规则、质量门禁、通知模板保持不变；创建人默认作为拥有人，创建人不可用时由关联应用 Owner 或平台管理员补齐。
...
- 上线后按流水线应用和代码源仓库自动绑定代码扫描配置；未匹配到时，自动创建当前应用的基础扫描配置。
- 存量任务中的语言、模块、扫描路径、红线和质量门禁阻断配置保持不变。
...
| 当前流水线应用未配置代码扫描 | 任务抽屉展示快速创建入口，创建当前应用的基础扫描配置 |
| 存量代码扫描配置 | 不要求业务方重建，上线后做兼容初始化 |
| 存量流水线扫描任务 | 自动绑定或创建当前应用的基础扫描配置 |
...
- 流水线已有 `SonarQube 代码扫描` 任务支持选择代码扫描配置。
- 任务不需要选择代码仓库。
- 任务抽屉不展示通知人字段。
- 质量门禁失败时阻断开关生效。
